/**
 * PhotographerWorkWidget - Manual Work Entry Widget for Photographers
 * Allows photographers to manually enter what they're working on when on a shoot
 * Displays on PhotographerDashboard, ShootsPage, and LeadDashboard
 */
...
export default function PhotographerWorkWidget({
  ...
  // Load existing notes from photographer attendance
  useEffect(() => {
    if (activeShoot?.notes) {
      setNotes(activeShoot.notes);
    } else {
      setNotes('');
    }
    setIsEditing(false);
  ...
  if (!activeShoot && !readOnly) {
    return null; // Don't show widget if no active shoot
  }
